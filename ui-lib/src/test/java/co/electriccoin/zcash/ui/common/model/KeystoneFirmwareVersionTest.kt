...
class KeystoneFirmwareVersionTest {
    ...
    fun minimumSupportedMatchesProductRequirement() {
        // MOB-1510 sets the regular-send minimum at 3.0.1 — keep in sync with iOS
        // `KeystoneFirmwareVersion.minimumSupported`.
        assertEquals(KeystoneFirmwareVersion(3, 0, 1), KeystoneFirmwareVersion.MINIMUM_SUPPORTED)
    }

    @Test
    fun toStringRendersDottedTriple() {
        assertEquals("3.0.1", KeystoneFirmwareVersion(3, 0, 1).toString())
    }
